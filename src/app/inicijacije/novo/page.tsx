...
async function createDeal(formData: FormData) {
  ...
  const krajnji_klijent_id_raw = String(formData.get("krajnji_klijent_id") ?? "").trim();
  const krajnji_klijent_id = krajnji_klijent_id_raw ? Number(krajnji_klijent_id_raw) : null;

  const radni_naziv = String(formData.get("radni_naziv") ?? "").trim();
  const napomena = String(formData.get("napomena") ?? "").trim() || null;

  if (!Number.isFinite(narucilac_id) || narucilac_id <= 0) redirect("/inicijacije/novo");
...
export default async function NewDealPage() {
  ...
  return (
    <div className="container">
      ...
      <form action={createDeal} className="card" style={{ padding: 14, maxWidth: 860 }}>
        <div style={{ display: "grid", gap: 10 }}>
          <div>
            <div className="muted" style={{ marginBottom: 6 }}>Naručilac (obavezno)</div>
            <select
              name="narucilac_id"
              required
              style={{
          ...
          <div>
            <div className="muted" style={{ marginBottom: 6 }}>Krajnji klijent (opcionalno)</div>
            <select
          ...
          <div>
            <div className="muted" style={{ marginBottom: 6 }}>Radni naziv (obavezno)</div>
            <input
              name="radni_naziv"
              required
          ...
          <div>
            <div className="muted" style={{ marginBottom: 6 }}>Napomena (opcionalno)</div>
            <textarea
              name="napomena"
              rows={4}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <a href="/inicijacije" className="project-link" style={{ alignSelf: "center" }}>
              Odustani
            </a>
          </div>
        </div>
