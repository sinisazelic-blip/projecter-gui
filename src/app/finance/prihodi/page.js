...
export default async function PrihodiListPage({ searchParams }) {
  ...
  try {
    ...
    if (q) {
      where.push("(CAST(prihod_id AS CHAR) LIKE ? OR opis LIKE ? OR napomena LIKE ?)");
      params.push(`%${q}%`, `%${q}%`, `%${q}%`);
    }
    ...
    rows = await query(`SELECT * FROM projektni_prihodi ORDER BY prihod_id DESC LIMIT 200`, []);
  }
      ...
      <div className="card">
        <form className="card-row" method="GET" style={{ gap: 12, flexWrap: "wrap" }}>
          <div style={{ minWidth: 260 }}>
            ...
            <input className="input" name="q" defaultValue={q} placeholder="ID / opis…" />
          </div>

          <div style={{ width: 180 }}>
            <div className="label">projekat_id</div>
            <input className="input" name="projekat_id" defaultValue={projekatId} placeholder="npr. 77" />
          </div>

          <div style={{ alignSelf: "flex-end", display: "flex", gap: 8 }}>
            <button className="btn btn-primary" type="submit">Primijeni</button>
            <Link className="btn" href="/finance/prihodi">Reset</Link>
          </div>
        </form>
      </div>
              ...
              {list.length ? (
                list.map((r) => {
                  ...
                  return (
                    <tr key={id}>
                      <td>
                        <Link className="link" href={`/finance/prihodi/${id}`}>{id}</Link>
                      </td>
                      <td>{r.projekat_id ?? "—"}</td>
