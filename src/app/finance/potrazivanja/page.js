    ...
    <div className="container">
      ...
      <div className="card">
        <form className="card-row" method="GET" style={{ gap: 12, flexWrap: "wrap" }}>
          <div style={{ minWidth: 260 }}>
            ...
            <input className="input" name="q" defaultValue={q} placeholder="ID / projekat / opis…" />
          </div>

          <label className="subtle" style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 22 }}>
            <input type="checkbox" name="only_open" value="1" defaultChecked={onlyOpen} />
            Samo otvorena (iznos − paid &gt; 0)
          </label>

          <div style={{ alignSelf: "flex-end", display: "flex", gap: 8 }}>
            <button className="btn btn-primary" type="submit">Primijeni</button>
            <Link className="btn" href="/finance/potrazivanja">Reset</Link>
          </div>
        </form>
      </div>
      ...
      <div className="card">
        <div className="card-row" style={{ justifyContent: "space-between" }}>
          ...
          <div className="subtle">Kolona “Banka” otvara filter na /finance/banka</div>
        </div>
            ...
            <tbody>
              {list.length ? (
                list.map((r) => {
                  ...
                  const paid = Number(r.paid_km ?? 0);
                  const rem =
                    Number.isFinite(iznos) && Number.isFinite(paid) ? iznos - paid : null;
                  ...
                  return (
                    <tr key={id}>
                      <td>
                        <Link className="link" href={`/finance/potrazivanja/${id}`}>{id}</Link>
                      </td>
                      <td>{proj ?? "—"}</td>
                      <td>{fmtDate(r.datum)}</td>
                      <td>{fmtDate(r.datum_dospijeca)}</td>
                      <td style={{ textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtKM(iznos)}</td>
                      <td style={{ textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{fmtKM(paid)}</td>
                      ...
                      <td>
                        ...
                        {r.napomena ? <div className="subtle">{r.napomena}</div> : null}
                        {rem !== null ? (
                          <div className="subtle" style={{ marginTop: 6 }}>
                            {rem <= 0.0001 ? badge("ZATVORENO", "ok") : badge("OTVORENO", "warn")}
                          </div>
                        ) : null}
                      </td>
                    </tr>
