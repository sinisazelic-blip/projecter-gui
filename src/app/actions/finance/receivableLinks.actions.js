...
async function syncReceivableStatus(potrazivanje_id) {
  const sums = await getPaidSum(potrazivanje_id);
  // zbog decimala: tolerancija 0.01
  const remaining = Number.isFinite(sums.remaining_km) ? sums.remaining_km : 0;
  const newStatus = remaining <= 0.01 ? "PAID" : "OPEN";
...
export async function financeLinkIncomeToReceivable({
  ...
  const cap = Math.round(Number(pot.iznos_km) * 100) / 100;
