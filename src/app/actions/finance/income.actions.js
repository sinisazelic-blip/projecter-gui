...
export async function financeCreateProjectIncome({
  ...
  const orig =
    iznos_original === null ||
    iznos_original === undefined ||
    String(iznos_original).trim() === ""
      ? null
      : Number(iznos_original);

  const val =
    valuta_original === null ||
    valuta_original === undefined ||
    String(valuta_original).trim() === ""
      ? null
  ...
  const r = await query(
    ...
    [
      ...
      d,
      o,
      Number.isFinite(orig) ? orig : null,
      val,
      Number.isFinite(kurs) ? kurs : null,
      km,
      et,
      eid,
    ],
