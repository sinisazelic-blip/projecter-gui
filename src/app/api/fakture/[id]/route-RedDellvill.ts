...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const fakturaIdParam = p.id;
  try {
    const fakturaId = Number(fakturaIdParam);
    ...
    console.error(`❌ Greška u API /api/fakture/${fakturaIdParam}:`, err?.message);
    console.error(`   Stack:`, err?.stack);
    return NextResponse.json(
      { 
        ok: false, 
