...
import { parseNovaPdf } from "@/lib/bank/parseNovaPdf";
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    hint: "POST multipart/form-data: file=<nova-banka.pdf>, account_id=<int>, mode=staging|final",
  });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name.endsWith(".pdf") && file.type !== "application/pdf") {
      return NextResponse.json(
        { ok: false, error: "Očekujem PDF fajl (Nova Banka izvod)." },
        { status: 400 },
      );
    }
    ...
    const file_hash = sha256(pdfBuf);

    const parsed = await parseNovaPdf(pdfBuf);
    const batch = parsed.batch;
    const txs = parsed.txs;
    const source = "NOVA_PDF";
    const currency = "BAM";
