...
| Zadatak | Opis |
|--------|------|
| i18n biblioteka | next-intl ili react-i18next (ili sl.) integrisana u Next.js |
| Fajlovi prevoda | npr. `locales/sr.json`, `locales/en.json` – svi UI stringovi (dugmad, labele, poruke, meni) |
| Language switcher | Komponenta u header-u (ili postavkama) za izbor jezika; preferenca u cookie/localStorage |
| Format | Datum, broj, valuta – po jeziku/regionu (locale) |

### 1.3 Napomena

- Kompletna zamjena teksta kroz app preko ključeva (npr. `t('common.save')`) – posao je u obimu, ali jednokratan; kasnije dodavanje jezika je samo novi JSON.
