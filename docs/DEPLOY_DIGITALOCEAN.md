...
1. **Lokalno** u rootu projekta (`FLUXA`) završiš izmjene i commit-uješ na granu **`main`**.
2. **Push na GitHub:**  
   `git push origin main`  
   Repozitorij: `https://github.com/sinisazelic-blip/fluxa.git`
...
```powershell
cd "C:\Users\Studio\OneDrive\Desktop\FLUXA"
git status
git add -A
...
cd /putanja/do/FLUXA   # gdje je repo
git fetch origin
git checkout main
...
- Na GitHubu (sinisazelic-blip) napravi **novi prazan repozitorijum** (npr. `fluxa`). Ne dodavaj README/licence (već postoje u projektu).

## 2. Lokalno: poveži i push

U rootu projekta (`FLUXA`) pokreni (zamijeni `TvojGitHubUsername` ako koristiš drugi):

```powershell
cd "c:\Users\Studio\OneDrive\Desktop\FLUXA"

# Poveži GitHub repo (ako si napravio fluxa)
git remote add origin https://github.com/sinisazelic-blip/fluxa.git
...
- **Repo:**  
  `https://github.com/sinisazelic-blip/fluxa.git`  
