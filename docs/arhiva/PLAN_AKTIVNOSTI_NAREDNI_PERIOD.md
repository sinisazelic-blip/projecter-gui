...
**Šta nam je ostalo (pregled):**

1. **Fiskalni uređaj (2.1)** – kasnije, kad stignu dodatna pojašnjenja od developera/servisera.
2. **Dokumentacija i User manual (2.5)** – korak-po-korak za nove korisnike.
3. **Export prazne SQL baze (2.7)** – ostavljeno za kraj (kad sve ostale faze budu završene).

*(Tooltip (2.6) i User management + Saradnik (2.4) su u potpunosti završeni. Za 2.4: zaštita ruta po matrici, Guest samo Dashboard, Saradnik nivo 0 – samo PP i svoji projekti, limit saradnika pri dodavanju/izmjeni. Pokreni scripts/add-role-saradnik.sql da dodaš ulogu Saradnik u bazu.)*
